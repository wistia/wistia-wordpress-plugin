...
function wistia_wordpress_settings_init() {
	...
	add_settings_field(
		'anti_mangler',
		esc_html_( 'Include Legacy Anti-Mangler', 'wistia' ),
...
function wistia_wordpress_anti_mangler_render() {
	...
	$on = empty( $wistia_wordpress_anti_mangler ) || 'on' === $wistia_wordpress_anti_mangler;
	?>
	<label>
...
function wistia_wordpress_settings_section_callback() {
	esc_html_e( '<p>The Wistia WordPress plugin enables oEmbeds to work in WordPress.</p> <p>There is also some legacy functionality which allows raw HTML, which we recommend turning OFF. It\'s better to use oEmbed!</p>', 'wistia' );
}
